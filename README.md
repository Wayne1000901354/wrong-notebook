...
- **📄 错题列表分页**：新增分页组件，支持大量错题的分页浏览，提升加载性能和用户体验。
- **🔍 知识点筛选修复**：修复了章节/知识点二级筛选不生效的问题，筛选功能更加精准。
- **📝 自定义日志系统**：从 pino 迁移到轻量级自定义日志实现，解决 Turbopack 兼容性问题。
  - 开发环境：终端美化输出，支持颜色和格式化。
  - 生产环境：结构化 JSON 日志，便于日志收集和分析。
- **🎯 AI 日志增强**：增强了 Gemini/OpenAI 调用的日志记录能力，便于调试和问题定位。
- **🖥️ 前端日志批量发送**：实现了前端日志的批量收集和发送机制，减少网络请求。
- **⚠️ 错误提示优化**：修正了 API Key 错误时的提示信息，显示更准确的异常来源。
